...
# 3DE4.script.comment:	Nudges translation, rotation and scale, of Cameras, Objects, 3D Models or Motion paths, in Local, Global, Screen or Lineup space
			...
			if r_space == 3:
				if len(pl)>0:
					...
					for point in pl:
						...
						l.append(p_name)
					tde4.setWidgetValue(self.req,"pivot_3dpoint",str(l))
				else:
					tde4.setWidgetValue(self.req,"pivot_3dpoint"," ")
			else:
				tde4.setWidgetValue(self.req,"pivot_3dpoint"," ")
		if s_type == 2:	
	...
	def Pivot_Menu_Edit(self,requester,widget,action):
		...
		if  s_type == 3:
			...
			if t_space == 3 or r_space == 3:
				...
				tde4.setWidgetValue(self.req,"look"," ")										
		if  s_type == 4:
			tde4.modifyOptionMenuWidget(self.req,"t_space","Pivot Space","Global - non functional WIP","Local - non functional WIP","Screen - non functional WIP","Lineup - non functional WIP")
			tde4.modifyOptionMenuWidget(self.req,"r_space","Pivot Space","Global - non functional WIP","Local - non functional WIP","Screen - non functional WIP","Lineup - non functional WIP")
			tde4.setWidgetValue(self.req,"sel"," ")
			tde4.setWidgetValue(self.req,"look"," ")
		if  s_type == 5:			
			pl = tde4.getPointList(pg,1)
			tde4.modifyOptionMenuWidget(self.req,"t_space","Pivot Space","Screen ")			
			if len(pl)>0:
				l = []
				for point in pl:
					p_name = tde4.getPointName(pg,point)
...
#disable/enable scale buttons...	
		if s_type == 1 or s_type == 2 or s_type == 5:
		...
		if s_type == 3 or s_type == 4 or s_type == 5:
			tde4.setWidgetSensitiveFlag(self.req,"rot_x+",1)
			tde4.setWidgetSensitiveFlag(self.req,"rot_x-",1)
			tde4.setWidgetSensitiveFlag(self.req,"rot_y+",1)
			tde4.setWidgetSensitiveFlag(self.req,"rot_y-",1)
		...
		if s_type == 5:
			tde4.setWidgetSensitiveFlag(self.req,"rot_x+",0)
			tde4.setWidgetSensitiveFlag(self.req,"rot_x-",0)
			tde4.setWidgetSensitiveFlag(self.req,"rot_y+",0)
			tde4.setWidgetSensitiveFlag(self.req,"rot_y-",0)
			tde4.setWidgetSensitiveFlag(self.req,"rot_z+",0)	
			tde4.setWidgetSensitiveFlag(self.req,"rot_z-",0)
			tde4.setWidgetSensitiveFlag(self.req,"r_increment",0)
			tde4.setWidgetSensitiveFlag(self.req,"r_increment_factor",0)
			tde4.setWidgetSensitiveFlag(self.req,"r_space",0)										
			tde4.setWidgetSensitiveFlag(self.req,"scale_x+",0)
			tde4.setWidgetSensitiveFlag(self.req,"scale_x-",0)
			tde4.setWidgetSensitiveFlag(self.req,"scale_y+",0)
			tde4.setWidgetSensitiveFlag(self.req,"scale_y-",0)
...
#rotate camera around 3dpoint(changes camera pivot to selected 3dpoint)...
					if r_space == 3:
						if widget == "rot_x+" or widget == "rot_x-" or widget == "rot_y+" or widget == "rot_y-" or widget == "rot_z+" or widget == "rot_z-":						
							#get average of 3dpoints positions..
							if len(pl) > 0:	
								x_list = []
								y_list = []
								z_list = []
								...
								tde4.postQuestionRequester(self.window_title,"Error, atleast one 3dpoint must be selected.","Ok")								
			else:
							...
							if len(pl) > 0:	
								...
								for m1 in mlist:
									if tde4.get3DModelName(pg,m1) == "patcha":
										model = m1
										break
								objpos = tde4.getPGroupPosition3D(pg,cam,frame)
								objrot = tde4.getPGroupRotation3D(pg,cam,frame)
								#update mpos and mrot
								mpos = tde4.get3DModelPosition3D(pg,model,cam,frame)
								mrot = tde4.get3DModelRotationScale3D(pg,model)
								#move 3dmodel pivot...
								...
								rot = rot * r
								tde4.set3DModelRotationScale3D(pg,model,rot.list())	
								#separate 3dmodel rotation to apply on objectPGroup...
								m = mat3d(tde4.get3DModelRotationScale3D(pg,model)).trans()
								s        = vec3d(m[0].norm2(),m[1].norm2(),m[2].norm2())
								r_matrix        = mat3d(m[0].unit(),m[1].unit(),m[2].unit()).trans()
								#update mpos and mrot
								...
								#bake 3dmodel first vertex to global space...
								com_global = (mat3d(mrot)*vec3d(tde4.get3DModelVertex(pg,model,0,cam,frame))) + vec3d(mpos)
								#addition objectPGroup local(child space) position to baked vertex position...
								newvalues = tde4.convertObjectPGroupTransformationWorldTo3DE(cam,frame,objrot,objpos,1.0,0)
								x = newvalues[1][0] + com_global[0]
								y = newvalues[1][1] + com_global[1]
								z = newvalues[1][2] + com_global[2]
								#take objectPGroup rotation degrees...
								o_rot	= rot3d(mat3d(newvalues[0])).angles(VL_APPLY_ZXY)
								o_rx	= (o_rot[0]*180.0)/3.141592654
								o_ry	= (o_rot[1]*180.0)/3.141592654
								o_rz	= (o_rot[2]*180.0)/3.141592654
								#take 3DModel rotation degrees...
								m_rot	= rot3d(mat3d(r_matrix)).angles(VL_APPLY_ZXY)
								m_rx	= (m_rot[0]*180.0)/3.141592654
								m_ry	= (m_rot[1]*180.0)/3.141592654
								m_rz	= (m_rot[2]*180.0)/3.141592654
								...
								rx	= (rx*3.141592654)/180.0
								ry	= (ry*3.141592654)/180.0
								rz	= (rz*3.141592654)/180.0
								r3d	= mat3d(rot3d(rx,ry,rz,VL_APPLY_ZXY))


								tde4.setPGroupPosition3D(pg,cam,frame,[x,y,z])
...
#aim math for screen Z...							
							if widget == "pos_z+" or widget == "pos_z-":
								if widget == "pos_z-": power = -float(power)
								cam_pos = obj_rot.trans()*(cam_pos - obj_pos)
								z_axis	= cam_pos+((mpos-cam_pos)*(1.0+float(power)))
								z_axis = vec3d(z_axis) + vec3d(vector)
								#convert 3DModel global to local...
								mpos = mat3d(obj_rot).invert() * vec3d(z_axis)
								#tde4.set3DModelPosition3D(pg,model,mpos.list())
								#print z_axis
								print mpos
								print z_axis
...
#push points.. 	
		if s_type == 5:	
			if len(pl) > 0:
				...
				tde4.postQuestionRequester(self.window_title,"atleast one 3D point must be selected","OK")					
...
#build GUI....
	def GUI(self):
		self.window_title = "Patcha Nudge3D Tool v1.0b2.94"
		self.req = tde4.createCustomRequester()
#type widget...
		tde4.addOptionMenuWidget(self.req,"selected_menu","Selected Type","Camera","Object PGroup","3D Model","Motion Trail","3D Points")
#sep1 widget...
...
#translation pivot widgets...
		...
		tde4.addScaleWidget(self.req,"t_increment"," ","DOUBLE",0.001,10.0,1.0)
		...
		tde4.addTextFieldWidget(self.req,"t_increment_factor"," ","1.0")
...
#rotation pivot widgets...
		...
		tde4.addScaleWidget(self.req,"r_increment"," ","DOUBLE",0.001,10.0,1.0)
		...
		tde4.addTextFieldWidget(self.req,"r_increment_factor"," ","1.0")
...
#scale button widgets...
		...
		tde4.setWidgetSensitiveFlag(self.req,"uniform_scale+",0)
		tde4.addScaleWidget(self.req,"s_increment"," ","DOUBLE",0.001,10.0,0.1)
		...
		tde4.setWidgetSensitiveFlag(self.req,"s_increment",0)
		tde4.addTextFieldWidget(self.req,"s_increment_factor"," ","0.1")
...
#pivot on 3dpoint...
		tde4.addTextFieldWidget(self.req,"pivot_3dpoint","Camera Pivot on 3DPoints"," ")
		tde4.setWidgetAttachModes(self.req,"pivot_3dpoint","ATTACH_POSITION","ATTACH_POSITION","ATTACH_WINDOW","ATTACH_AS_IS")
		tde4.setWidgetOffsets(self.req,"pivot_3dpoint",53,98,517,0)
...
#pivot menu callback...
		...
		tde4.postCustomRequesterAndContinue(self.req,self.window_title,340,545,"do.update")
