...
window_title = "Patcha Pose Saver/Loader v1.4.8"
...
def ImportCameraPose():
	...
	if pg!=None and cam !=None:
		if tde4.getPGroupType(pg)=="CAMERA" or tde4.getCameraType(cam) =="REF_FRAME":
			...
			tde4.postQuestionRequester(window_title,"CameraPGroup & Camera must be selected.", "ok")						
	else:
...
def Apply(req,widget,action):
	...
	frame = tde4.getCurrentFrame(cam)
	pg_type = tde4.getPGroupType(pg)
	frame_range_calc = tde4.getCameraFrameRangeCalculationFlag(cam)
	if widget == "apply":
		...
		if menu1_value == 1 and menu2_value == 1:
			if frame_range_calc == 0:
				ImportCameraPose()
			else:
				tde4.postQuestionRequester(window_title, "Error, 'Frame Range Calculation' is turned on, please turn off that.", "ok")				
...
tde4.addOptionMenuWidget(req,"mode_menu1","Select Type","Camera","ObjectPGroup","3D Model")
