...
# Might be broken, checks whether Focus is 1:1, if so, bakes only SqX/Y
# If not 1:1, prebakes all distortion curves
...
def Convert(req,widget,action):
	...
	#create emphty lists...
		...
		#function to bake focal to sqx&y...				
		def Focal_to_Sqxy():						
			focal_list = []
			new_sq_x_list = []
			new_sq_y_list = []
		...
		if widget == "focal_to_sqxy":
			#check focus curve is linear or non-linear...
			count = 0
			for i in range(1,frames+1):
				focus = tde4.getCameraFocus(cam,i)
				if focus == i:
					count = count + 1
			if count == frames:
				#print "linear"
...
#build GUI...
window_title = "FocalLength-SqueezeX/Y Converter v1.2"
try:
