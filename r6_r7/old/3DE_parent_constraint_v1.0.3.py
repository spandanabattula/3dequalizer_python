	...
	if widget == "constraint_btn":
		...
		if offset_v == 1:
			...
			for frame in range(current_frame,end_frame):
				...
				parent_pg_pos = tde4.getPGroupPosition3D(parent_pg,cam,frame+1)
				parent_pg_rot = tde4.getPGroupRotation3D(parent_pg,cam,frame+1)


				pivot = tde4.getPGroupPosition3D(parent_pg,cam,frame+1)				

				parent_rot3d_current_frame = tde4.getPGroupRotation3D(parent_pg,cam,frame)
				parent_rot3d_next_frame = tde4.getPGroupRotation3D(parent_pg,cam,frame+1)

				parent_rot3d_current_frame = vec3d(convertToAngles(parent_rot3d_current_frame))
				parent_rot3d_next_frame = vec3d(convertToAngles(parent_rot3d_next_frame))

				parent_rot3d_delta =  parent_rot3d_next_frame - parent_rot3d_current_frame 

				delta_rot_x, delta_rot_y, delta_rot_z = parent_rot3d_delta



				child_pos = vec3d(tde4.getPGroupPosition3D(child_pg,cam,frame+1))
				child_rot = mat3d(tde4.getPGroupRotation3D(child_pg,cam,frame+1))

				new_pos3d = rot3d(delta_rot_x,delta_rot_y,delta_rot_z,VL_APPLY_ZXY) * (child_pos - pivot) + pivot
				#tde4.setPGroupPosition3D(pg,cam,frame,p3d.list())
				x_axis = vec3d(1,0,0)
				y_axis = vec3d(0,1,0)
				z_axis = vec3d(0,0,1)
				#r = mat3d(rot3d(z_axis,math.radians(float(delta_rot_z)))) * mat3d(rot3d(y_axis,math.radians(float(delta_rot_y)))) * mat3d(rot3d(x_axis,math.radians(float(delta_rot_x))))
				r =  mat3d(rot3d(x_axis,math.radians(float(delta_rot_x))))				
				new_rot3d = mat3d(r * child_rot)
				#tde4.setPGroupRotation3D(pg,cam,frame,rot.list())


				if tde4.getPGroupType(child_pg) == "OBJECT":
					world_to_3de = tde4.convertObjectPGroupTransformationWorldTo3DE(cam,frame+1,new_rot3d.list(),list(new_pos3d),1.0,1)
					parent_pos3d = world_to_3de[1]
					parent_rot3d = world_to_3de[0]
				if tde4.getPGroupType(child_pg) == "CAMERA":
					parent_pos3d = new_pos3d
					parent_rot3d = new_rot3d
				tde4.setPGroupPosition3D(child_pg,cam,frame+1,list(parent_pos3d))
				tde4.setPGroupRotation3D(child_pg,cam,frame+1,parent_rot3d)
				tde4.setPGroupScale3D(child_pg,tde4.getPGroupScale3D(child_pg))

				if tde4.getPGroupType(child_pg) == "OBJECT":
					tde4.makePGroupChildSpaceCurvesConsistentToGlobalSpaceCurves(child_pg)
					tde4.copyPGroupEditCurvesToFilteredCurves(child_pg,cam)				
...
window_title = "Patcha PGroup Parent Constraint Tool v1.0.3"
