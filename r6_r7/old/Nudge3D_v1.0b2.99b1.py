...
# 3DE4.script.version:	 v1.0b2.99b1
			...
			if pg_type == "CAMERA":
				if pg != None:
					...
					if toggle_value == 0:
						...
						loop_end = (frame)+offset
						key_frame = frame+offset
					if toggle_value == 1:
						loop_start = tde4.getWidgetValue(self.req,"frame_in")
						loop_end = tde4.getWidgetValue(self.req,"frame_out")
						key_frame = tde4.getWidgetValue(self.req,"set_key_frame")
					if p_space == 1:
#camera global translation nudge...
						if widget == "pos_x+" or widget == "pos_x-" or widget == "pos_y+" or widget == "pos_y-" or widget == "pos_z+" or widget == "pos_z-":
							if loop_start != None and loop_end != None and (int(loop_start)-offset) >= 1 and (int(loop_start)-offset) <= frames and (int(loop_end)-offset) >= 1 and (int(loop_end)-offset) <= frames and (int(loop_start)-offset) <= (int(loop_end)-offset):
								if key_frame != None:
									#store set key frame pos & rot...
									set_key_cam_pos = vec3d(tde4.getPGroupPosition3D(pg,cam,int(key_frame)-offset))
									set_key_cam_rot = mat3d(tde4.getPGroupRotation3D(pg,cam,int(key_frame)-offset))
									if toggle_value == 0:
										power = tde4.getWidgetValue(self.req,"t_increment_factor")
									if toggle_value == 1:
										power = tde4.getWidgetValue(self.req,"t_increment_factor")
										power = float(power)/((int(loop_end) - int(loop_start)) + 0)
									count = 0									
									for frame in range(int(loop_start)-offset,(int(loop_end)+1)-offset):
										...
										v = float(power) * float(count)

										if widget == "pos_x+" or widget == "pos_x-":
											if widget == "pos_x-": v = -float(v)
											pos_x = float(cam_pos[0]) + float(v)
											tde4.setPGroupPosition3D(pg,cam,frame,[pos_x,cam_pos[1],cam_pos[2]])
										if widget == "pos_y+" or widget == "pos_y-":
											if widget == "pos_y-": v = -float(v)
											pos_y = float(cam_pos[1]) + float(v)
											tde4.setPGroupPosition3D(pg,cam,frame,[cam_pos[0],pos_y,cam_pos[2]])
										if widget == "pos_z+" or widget == "pos_z-":
											if widget == "pos_z-": v = -float(v)
											pos_z = float(cam_pos[2]) + float(v)
											tde4.setPGroupPosition3D(pg,cam,frame,[cam_pos[0],cam_pos[1],pos_z])
										tde4.filterPGroup(pg,cam)
									#if frame range toggle is turned on then revert back to set key frame pos & rot...
									if toggle_value == 1:
										tde4.setPGroupPosition3D(pg,cam,(int(key_frame)-offset),set_key_cam_pos.list())
										tde4.setPGroupRotation3D(pg,cam,(int(key_frame)-offset),set_key_cam_rot.list())
									tde4.filterPGroup(pg,cam)
								else:
									tde4.postQuestionRequester(self.window_title,"Error, please do set key first.","Ok")
							else:
...
#build GUI....
	def GUI(self):
		self.window_title = "Patcha Nudge3D Tool v1.0b2.99b1"
...
#frame out widget...
		...
		tde4.setWidgetOffsets(self.req,"toggle",38,44,610,0)
#set key button widget...
		tde4.addButtonWidget(self.req,"set_key","set key",70,10)
		tde4.setWidgetAttachModes(self.req,"set_key","ATTACH_POSITION","ATTACH_POSITION","ATTACH_WINDOW","ATTACH_AS_IS")
		tde4.setWidgetOffsets(self.req,"set_key",48,70,610,0)
#set key text field widget...
		...
		tde4.setWidgetOffsets(self.req,"set_key_frame",74,98,610,0)	
...
#set key frame callback...
		...
		tde4.postCustomRequesterAndContinue(self.req,self.window_title,390,640,"do.update")
