...
def mainCallback(req,widget,action):
	...
	if widget == "constraint_btn":
		...
		if offset_v == 1:
			...
			for frame in range(current_frame,frames):
				parent_pos3d = vec3d(tde4.getPGroupPosition3D(parent_pg,cam,frame+1))
				...
				parent_rot3d_delta = mat3d(parent_rot3d_current_frame).invert() * mat3d(parent_rot3d_next_frame)
				#parent_rot3d_delta =  mat3d(parent_rot3d_next_frame) * mat3d(parent_rot3d_current_frame).invert()
				#parent_rot3d_delta =  mat3d(parent_rot3d_next_frame).invert() * mat3d(parent_rot3d_current_frame)
				#parent_rot3d_delta =  mat3d(parent_rot3d_current_frame) * mat3d(parent_rot3d_next_frame).invert()




				parent_rot3d_delta = list(convertToAngles(parent_rot3d_delta))


				rx = float(parent_rot3d_delta[0] * 180.0) / math.pi 
				ry = float(parent_rot3d_delta[1] * 180.0) / math.pi 
				rz = float(parent_rot3d_delta[2] * 180.0) / math.pi 
				...
				#child_pos3d = vec3d(tde4.getPGroupPosition3D(child_pg,cam,frame))
				new_pos3d = parent_rot3d_delta * (child_pos3d - pivot) + pivot
...
window_title = "Patcha Parent Constraint Tool v1.0.1"
