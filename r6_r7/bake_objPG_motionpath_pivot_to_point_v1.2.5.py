...
def Update(req):
	...
	if tde4.getPGroupType(pg) == "OBJECT":
		...
		tde4.setWidgetSensitiveFlag(req,"create_locator_btn",0)
		tde4.setWidgetSensitiveFlag(req,"delete_locator_btn",0)
		tde4.setWidgetSensitiveFlag(req,"move_path_point_btn",0)
		tde4.setWidgetSensitiveFlag(req,"move_path_locator_btn",0)
...
def Pick_Point(req,widget,action):
	...
	if tde4.getPGroupType(pg) == "OBJECT":
		if len(pl) == 1:
			...
			tde4.setWidgetSensitiveFlag(req,"create_locator_btn",1)
			tde4.setWidgetSensitiveFlag(req,"delete_locator_btn",1)
			tde4.setWidgetSensitiveFlag(req,"move_path_point_btn",1)
			tde4.setWidgetSensitiveFlag(req,"move_path_locator_btn",1)
		else:
...
def createLocator(req,widget,action):
	pg = tde4.getCurrentPGroup()
	...
	if point_name_given != " ":
		...
		for model in model_list:
			if tde4.get3DModelName(pg,model) == "objPG_pivot_locator":
				tde4.delete3DModel(pg,model)
		m = tde4.create3DModel(pg,7)
		tde4.set3DModelName(pg,m,"objPG_pivot_locator")
		vertex = 3.0
		tde4.add3DModelVertex(pg,m,[0.0,0.0,0.0])
		tde4.add3DModelVertex(pg,m,[vertex,0.0,0.0])
		tde4.add3DModelVertex(pg,m,[-vertex,0.0,0.0])
		tde4.add3DModelVertex(pg,m,[0.0,vertex,0.0])
		tde4.add3DModelVertex(pg,m,[0.0,-vertex,0.0])
		tde4.add3DModelVertex(pg,m,[0.0,0.0,vertex])
		tde4.add3DModelVertex(pg,m,[0.0,0.0,-vertex])
		for i in range(7):
			tde4.add3DModelLine(pg,m,[0,i])
		tde4.set3DModelSurveyFlag(pg,m,0)
		tde4.set3DModelPosition3D(pg,m,point_pos.list())
		tde4.set3DModelColor(pg,m,0.0,1.0,0.0,1.0)		
	else:
		tde4.postQuestionRequester(window_title, "Error, please pick 3d point first.", "ok")

def deleteLocator(req,widget,action):
	pg = tde4.getCurrentPGroup()
	model_list = tde4.get3DModelList(pg,0)
	for model in model_list:
		if tde4.get3DModelName(pg,model) == "objPG_pivot_locator":
...
def Move_motionPath(req,widget,action):
	...
	if widget == "move_path_point_btn" or widget == "move_path_locator_btn":
		...
		if tde4.getPGroupType(pg) == "OBJECT":		
			...
			point_name_given = str(tde4.getWidgetValue(req,"point_name_txt"))		
			if point_name_given != " ":
				#create pivot witness dummy point
				null = 0
				for p in pl:
					name = tde4.getPointName(pg,p)
					if name == "pivot_witness":
						null = 1
				if null == 0:
					point = tde4.createPoint(pg)
					tde4.setPointName(pg,point,"pivot_witness")
					tde4.setPointSurveyMode(pg,point,"SURVEY_EXACT")
					tde4.setPointSurveyPosition3D(pg,point,[0.0,0.0,0.0])
					tde4.setPointHideFlag(pg,point,1)
					tde4.setPointCalcMode(pg,point,"CALC_OFF")
				...
				for frame in range(1,frames+1):	
					...
					if widget == "move_path_point_btn":
						point_pos = vec3d(tde4.getPointCalcPosition3D(pg,point))

					if widget == "move_path_locator_btn":
						mlist = tde4.get3DModelList(id_opg,0)
						for model in mlist:
							model_name = tde4.get3DModelName(id_opg,model)
							if model_name == "objPG_pivot_locator":
								point_pos = vec3d(tde4.get3DModelPosition3D(id_opg,model,cam,frame))
								break
							else:
								#tde4.postQuestionRequester(window_title, "Error, objPG_pivot_locator is not found.", "ok")
								point_pos = vec3d(0.0,0.0,0.0)
...
#GUI...
window_title = "Patcha Bake objectPG motionpath pivot to new point v1.2.5"
...
tde4.addButtonWidget(req,"create_locator_btn","Create pivot locator on point",70,10)
tde4.setWidgetAttachModes(req,"create_locator_btn","ATTACH_POSITION","ATTACH_POSITION","ATTACH_WINDOW","ATTACH_AS_IS")
tde4.setWidgetOffsets(req,"create_locator_btn",5,50,80,0)
tde4.setWidgetSensitiveFlag(req,"create_locator_btn",0)

tde4.addButtonWidget(req,"delete_locator_btn","Delete pivot locator",70,10)
tde4.setWidgetAttachModes(req,"delete_locator_btn","ATTACH_POSITION","ATTACH_POSITION","ATTACH_WINDOW","ATTACH_AS_IS")
tde4.setWidgetOffsets(req,"delete_locator_btn",55,95,80,0)
tde4.setWidgetSensitiveFlag(req,"delete_locator_btn",0)

tde4.addButtonWidget(req,"move_path_point_btn","Bake objectPG pivot to point",70,10)
tde4.setWidgetAttachModes(req,"move_path_point_btn","ATTACH_POSITION","ATTACH_POSITION","ATTACH_WINDOW","ATTACH_AS_IS")
tde4.setWidgetOffsets(req,"move_path_point_btn",5,50,110,0)
tde4.setWidgetSensitiveFlag(req,"move_path_point_btn",0)

tde4.addButtonWidget(req,"move_path_locator_btn","Bake objectPG pivot to locator",70,10)
tde4.setWidgetAttachModes(req,"move_path_locator_btn","ATTACH_POSITION","ATTACH_POSITION","ATTACH_WINDOW","ATTACH_AS_IS")
tde4.setWidgetOffsets(req,"move_path_locator_btn",55,95,110,0)
tde4.setWidgetSensitiveFlag(req,"move_path_locator_btn",0)

tde4.setWidgetCallbackFunction(req,"pick_point_btn","Pick_Point")
tde4.setWidgetCallbackFunction(req,"move_path_point_btn","Move_motionPath")
tde4.setWidgetCallbackFunction(req,"move_path_locator_btn","Move_motionPath")
tde4.setWidgetCallbackFunction(req,"create_locator_btn","createLocator")
tde4.setWidgetCallbackFunction(req,"delete_locator_btn","deleteLocator")


tde4.postCustomRequesterAndContinue(req,window_title,550,145,"Update")
