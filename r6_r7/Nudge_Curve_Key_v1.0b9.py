...
#help...				
	def Help(self,requester,widget,action):
		...
		tde4.appendTextAreaWidgetString(self.help_req,"textarea","Refresh button will update current selected keys info to the 'X' & 'Y' fields.\n\n")		
...
#build GUI...			
	def GUI(self):
		self.window_title = "Patcha Nudge Curve Key v1.0b9"
...
#update selected keys info while window opening...				
			cam = tde4.getCurrentCamera()
			frame = tde4.getCurrentFrame(cam)
			curve = tde4.getFirstCurrentCurve()
			if curve != None:
...
#widgetcallbacks...
			...
			tde4.postCustomRequesterAndContinue(self.req,self.window_title,440,220)
		else:
